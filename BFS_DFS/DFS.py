...
def depth_first_search(initialState: State):
    ...
    frontier = [(initialState, 0)]   # STACK for DFS
    explored = set()
    nodes_expanded = 0

    # LOOPING UNTIL THE FRONTIER IS EMPTY OR YOU REACH THE GOAL
    while frontier:
        state, depth = frontier.pop()   # LIFO → DFS
        # IF STATE ALREADY EXPLORED THEN SKIP
        if state in explored:
            continue
        # ADDING THE CURRENT STATE TO EXPLORED SET
        explored.add(state)
        nodes_expanded += 1
        ...
        if state.isGoal():
            ...
            while cur is not None:
                ...
                cur = cur.parent
            # REVERSING TO GET THE ORDER FROM ROOT TO GOAL
            path.reverse()
            moves.reverse()
        ...
        # DFS → push neighbors in reverse to maintain consistent order
